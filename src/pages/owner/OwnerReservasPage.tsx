...
import { Pencil, Plus, ClipboardList, Trash2, Star, Waves, ChevronDown } from "lucide-react";
...
const OwnerReservasPage = () => {
  ...
  useEffect(() => {
    if (routeState?.registerBoat) {
      navigate("/marinheiro/embarcacoes/novo", { replace: true });
      return;
    }
    if (routeState?.editBoatId && boats.length > 0) {
      const b = boats.find((x) => x.id === routeState.editBoatId);
      if (b) {
        iniciarEdicao(b);
        queueMicrotask(() => document.getElementById("owner-boats-manage")?.scrollIntoView({ behavior: "smooth" }));
      }
    }
  }, [routeState?.editBoatId, routeState?.registerBoat, boats, navigate]);
  ...
  return (
    <div
      className={cn(
        "space-y-4",
        "[&_#marinheiro-reservas]:rounded-xl [&_#marinheiro-reservas]:border [&_#marinheiro-reservas]:border-border/45 [&_#marinheiro-reservas]:bg-transparent [&_#marinheiro-reservas]:shadow-none",
        "[&_.surface-elevated]:rounded-xl [&_.surface-elevated]:border [&_.surface-elevated]:border-border/40 [&_.surface-elevated]:bg-transparent [&_.surface-elevated]:shadow-none",
        "[&_.surface-elevated]:dark:bg-transparent [&_.surface-elevated]:transition-colors",
        "[&_.surface-elevated:hover]:border-border/60"
      )}
    >
          ...
          <>
            ...
            >
              ...
              <div
                ...
                className="space-y-5 p-3 sm:p-4 block"
              >
            ...
            {false ? (
            <section id="owner-boats-manage" className="min-w-0 space-y-3 scroll-mt-24">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex min-w-0 flex-wrap items-center gap-2">
                  <h2 className="text-base font-semibold text-foreground sm:text-lg">{t("ownerPanel.myBoatsTitle")}</h2>
                      ...
                      <div className="space-y-1 sm:col-span-2">
                        ...
                        <div className="relative">
                          <Input
                            type="number"
                            min={1}
                            max={isMotoAquaticaVessel(newBoatForm.tipo) ? MOTO_AQUATICA_MAX_CAPACITY : undefined}
                            step={1}
                            className="pr-16"
                            placeholder={t("marinheiro.capacityPh")}
                            disabled={isMotoAquaticaVessel(newBoatForm.tipo)}
                            value={
                              isMotoAquaticaVessel(newBoatForm.tipo)
                                ? MOTO_AQUATICA_MAX_CAPACITY
                                : Math.max(1, newBoatForm.capacidade)
                            }
                            onChange={(e) =>
                              setNewBoatForm({
                                ...newBoatForm,
                              ...
                              <div className="space-y-1">
                                ...
                                <div className="relative">
                                  <Input
                                    ...
                                    step={1}
                                    className="pr-16"
                                    disabled={isMotoAquaticaVessel(boatForm.tipo)}
                                    value={
                                      isMotoAquaticaVessel(boatForm.tipo)
                                        ? MOTO_AQUATICA_MAX_CAPACITY
                                        : Math.max(1, boatForm.capacidade)
                                    }
                                    onChange={(e) =>
                                      setBoatForm({
                                        ...boatForm,
...
export default OwnerReservasPage;
