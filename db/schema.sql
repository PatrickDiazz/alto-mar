-- Alto Mar (PostgreSQL) - schema completo
-- Execução recomendada (psql):
--   psql -U postgres -f db/schema.sql
--
-- Observação: este arquivo inclui comandos do psql (`\\connect`).

-- 1) Criar banco
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_database WHERE datname = 'alto_mar') THEN
    EXECUTE 'CREATE DATABASE alto_mar';
  END IF;
END $$;
