...
/**
 * Marca reserva como paga a partir de um PaymentIntent confirmado (seed / testes automatizados).
 * Usado quando a Checkout Session ainda não tem PI (antes de abrir a URL hospedada).
 *
 * @param {import("pg").PoolClient} client
 * @param {{ bookingId: string; renterUserId: string; checkoutSessionId: string; pi: import("stripe").Stripe.PaymentIntent; ledgerEventId: string }} input
 */
export async function applyApprovedStripePaymentInTx(client, input) {
  const { bookingId, renterUserId, checkoutSessionId, pi, ledgerEventId } = input;
  const piId = pi.id;
  const ch = pi.latest_charge;
  const chargeId = typeof ch === "string" ? ch : ch && "id" in ch ? ch.id : null;
  const amountTotal = pi.amount_received ?? pi.amount;
  if (amountTotal == null) return;
  ...
  if (String(renterUserId) !== String(b.renter_user_id)) {
    const e = new Error("renter_user_id não coincide com a reserva.");
    e.code = "RENTER_MISMATCH";
    throw e;
  }
  ...
  await client.query(
    `update bookings
     set stripe_flow_status = $2,
         ...
         stripe_checkout_session_id = coalesce(stripe_checkout_session_id, $5)
     where id = $1::uuid`,
    [bookingId, StripeFlowStatus.PAID, platformFeeCents, ownerNetCents, checkoutSessionId]
  );
  ...
  const upPay = await client.query(
    `update payments
     set provider = 'STRIPE',
         ...
         updated_at = now()
     where booking_id = $1::uuid`,
    [bookingId, checkoutSessionId, piId, chargeId, Number(amountTotal)]
  );
  if (!upPay.rowCount) {
    await client.query(
      `insert into payments (
  ...
  await insertLedgerEntry(client, {
    ...
    runningBalanceCents: currentBalance + paid,
    eventId: ledgerEventId,
    description: "Pagamento recebido (Stripe seed test)",
    metadata: { checkout_session_id: checkoutSessionId, payment_intent_id: piId, seed: true },
  });
}
